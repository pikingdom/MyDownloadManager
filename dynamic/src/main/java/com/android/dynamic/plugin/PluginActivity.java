...
/**
 * 插件开发activity时,需要继承此类
 * 
 * @ClassName: PluginActivity
...
public class PluginActivity extends Activity implements IActivityPlugin {
	...
	public void startActivityForResult(Intent intent, int requestCode) {
		if (mPluginMode) {
			...
			if ((resolveInfo == null) || (resolveInfo.size() <= 0)) {
				intent.putExtra(PLUGIN_PKG_NAME, mPluginPackageName);
	...
	public ComponentName startService(Intent service) {
		if (mPluginMode) {
			...
			if ((resolveInfo == null) || (resolveInfo.size() <= 0)) {
				service.putExtra(PLUGIN_PKG_NAME, mPluginPackageName);
	...
	public boolean stopService(Intent name) {
		if (mPluginMode) {
			...
			if ((resolveInfo == null) || (resolveInfo.size() <= 0)) {
				name.putExtra(PLUGIN_PKG_NAME, mPluginPackageName);
